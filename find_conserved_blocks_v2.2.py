...
# Imported libraries
######################
# Must first install biopython library: >> pip/pip3 install biopython
...
# File data
######################
inputFileName = "recA_Mycobacteria_muscle_aignment_small.fasta"
inputFileType = "fasta"
alphabetFormat = IUPAC.unambiguous_dna
gapCharacter = "-"

inputFilePath = os.path.dirname(os.path.realpath(__file__)) + "/" + inputFileName
outputFilePath = inputFilePath + "_(" + str(matchingFraction) + "-" + str(minLength) + "-" + str(minSequences) + ").txt"
...
def isBlockConserved(blockSequence):
  maxAllowedMismatchedColumns = blockSequence.get_alignment_length() - int(blockSequence.get_alignment_length() * matchingFraction)
  mismatchedColumns = 0
...
for startSequence in range(numSequences - minSequences + 1):
  ...
  print("Start of sequence #" + str(startSequence + 1) + " (current time = " + str(int(current_time)) + "s)")
  for index in range(sequenceLength - minLength + 1):
    ...
    if not isBlockAlreadyCounted(startSequence, index, startSequence + sequences, index + length):
      ...
      while consensusRight:
        block = alignment[startSequence:(startSequence + sequences), index:(index + length)]
      ...
      if index != sequenceLength and length > minLength:
        ...
        largestConservedBlock, removedColumns = removeTrailingBlankColumns(largestConservedBlock)
        # Save block data in list
        conservedBlocks.append(largestConservedBlock)
        conservedBlocksInfo.append([startSequence, index, startSequence + sequences, index + length])
        # Print information about saved data
        print("Block found: " + str(conservedBlocks[-1].__len__()) + " sequences, " + str(conservedBlocks[-1].get_alignment_length()) + " long (from position " + str(index + 1) + " to " + str(index + length + 1) + ")")
...
for block in conservedBlocks:
  textFile.write("Number blank columns = " + str(getNumberBlankColumns(block)) + "\n")
  ...
  for row in range(block.__len__()):
    textFile.write("[" + str(conservedBlocksInfo[i][0] + row + 1) + "] " + block[row].name + "\n")
